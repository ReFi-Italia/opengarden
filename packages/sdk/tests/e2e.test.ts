...
function loadSchemaUIDs(): Partial<SchemaUIDs> | undefined {
  ...
  try {
    ...
    console.warn('  Warning: OPENGARDEN_SCHEMA_UIDS is not valid JSON, ignoring');
    return undefined;
  }
}
...
describe.skipIf(skip)('E2E: full intervention lifecycle', () => {
  ...
  let validationResult: TimestampedOffChainResult & { approved: boolean; qualityScore: number };
  ...
  beforeAll(async () => {
    ...
    client = new OpenGardenClient({ signer, chain, storage, schemaUIDs: cachedUIDs });

    console.log(`  Wallet: ${walletAddress}`);
    console.log(`  Chain:  ${CHAIN_NAME}`);
    console.log(`  RPC:    ${RPC_URL}`);
    console.log(`  Schema UIDs: ${cachedUIDs ? 'loaded from env' : 'will register on-chain'}`);

    const balance = await provider.getBalance(walletAddress);
    console.log(`  Balance: ${ethers.formatEther(balance)} ETH`);
    if (balance === 0n) throw new Error('Wallet has no testnet ETH — fund it first');
  }, 30_000);
  ...
  it('registers all schemas', async () => {
    ...
    if (results.length > 0) {
      ...
      console.log('  All schemas already registered (loaded from OPENGARDEN_SCHEMA_UIDS)');
    }
  ...
  it('builds and uploads evidence bundle', async () => {
    const bundle = client.buildEvidenceBundle({
      ...
      healthcheckBefore: {
        uid: healthcheckBeforeResult.uid,
        score: 3,
        onchainTimestamp: healthcheckBeforeResult.onchainTimestamp,
      },
      healthcheckAfter: {
        uid: healthcheckAfterResult.uid,
        score: 8,
        onchainTimestamp: healthcheckAfterResult.onchainTimestamp,
      },
    });
  ...
  it('reads back the intervention', async () => {
    ...
    console.log(`  Intervention read back: ${intervention.interventionId}, health ${intervention.healthBefore} → ${intervention.healthAfter}`);
  }, 30_000);
