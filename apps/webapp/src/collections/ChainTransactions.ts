...
export const ChainTransactions: CollectionConfig = {
  ...
  admin: {
    group: 'Settlement',
    useAsTitle: 'id',
    defaultColumns: ['kind', 'status', 'relatedCollection', 'relatedId', 'txHash'],
  },
  access: {
    read: authenticated,
    create: () => false,
  ...
  fields: [
    {
      ...
      type: 'select',
      required: true,
      options: CHAIN_TRANSACTION_KINDS.map((value) => ({ label: value, value })),
    },
    ...
    { name: 'attesterWallet', type: 'text' },
    {
      ...
      type: 'select',
      required: true,
      options: CHAIN_TRANSACTION_STATUS.map((value) => ({ label: value, value })),
    },
